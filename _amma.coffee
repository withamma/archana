...
app.config ["$stateProvider", "$urlRouterProvider", ($stateProvider, $urlRouterProvider) ->
  ...
  app.controller "MemorizeCtrl", ["$scope", '$stateParams', '$http', "$location",
      ...
      $scope.getHint = () ->
        if $scope.state is "show" then nextInLink(undefined).slice(0, 15) else ""
      ...
      $scope.linkPrevious = () ->
        if ($scope.state isnt "loading") then previousInLink(undefined) else "Loading"

      $scope.linkTest = () ->
        if ($scope.state is "loading")
          return "Loading"
        else if ($scope.state is "answer")
          return nextInLink(undefined)
        else
          return previousInLink(undefined)
  ...
  app.controller "HowtoCtrl", ["$scope", "$sessionStorage", '$stateParams', '$location',
    ($scope, storage, $state, $location) ->
      $scope.learn = () ->
        $location.path "/learn/#{$stateParams.itemId}"
      $scope.memorize = () ->
        $location.path "/memorize/#{$stateParams.itemId}"
  ]

  app.controller "LearnCtrl", ["$scope", "$localStorage", "$stateParams", "$http",
                               "hotkeys", "HistoryService", "mobile", "$location",
    ($scope, storage, $stateParams, $http, hotkeys, history, mobile, $location)->
      $scope.mobile = mobile
  ...
  app.controller "ResultsCtrl", ["$scope", "$localStorage", '$stateParams', '$http', 'dateFilter', "$window", "HistoryService",
    ($scope, storage, $stateParams, $http, dateFilter, $window, history) ->
